...
function handleVideoPlayback() {
    const video = document.getElementById('videoPath');
    ...
    if (video && backgroundMusic) {
        ...
        video.addEventListener('ended', () => {
            ...
            animationTimeline(); // Start animations after video ends
        });

        video.addEventListener('playing', () => {
            // Prevent animations from running during video playback
        });
    } else {
        console.error('Video or background music element not found');
    }
}
...
const animationTimeline = () => {
    ...
    if (textBoxChars && hbd) {
        ...
        console.error('Text box or HBD element not found');
        return;
    }

    // TimelineMax/GSAP animation sequence
    ...
    tl.to(".container", 0.6, { visibility: "visible" })
        ...
        .to(".idea-1", 0.7, { opacity: 0, y: 20, rotationY: 5, skewX: "-15deg" }, "+=2.5")
        .from(".idea-2", 0.7, { opacity: 0, y: -20, rotationX: 5, skewX: "15deg" })
        .to(".idea-2", 0.7, { opacity: 0, y: 20, rotationY: 5, skewX: "-15deg" }, "+=2.5")
        ...
        .to(".idea-3", 0.7, { opacity: 0, y: 20, rotationY: 5, skewX: "-15deg" }, "+=2.5")
        .from(".idea-4", 0.7, { opacity: 0, y: -20, rotationX: 5, skewX: "15deg" })
        .to(".idea-4", 0.7, { opacity: 0, y: 20, rotationY: 5, skewX: "-15deg" }, "+=2.5")
        ...
        .from(".profile-video", 0.5, { scale: 3.5, opacity: 0, x: 25, y: -25, rotationZ: -45 })
        .from(".hat", 0.5, { x: -100, y: 350, rotation: -180, opacity: 0 })
        .staggerFrom(".wish-hbd span", 0.7, { opacity: 0, y: -50, rotation: 150, skewX: "30deg", ease: Elastic.easeOut.config(1, 0.5) }, 0.1, "+=180")
    ...
    if (replyBtn) {
        replyBtn.addEventListener("click", () => {
            tl.restart();
            handleVideoPlayback();  // Reattach video listeners on restart
        });
    } else {
...
// Attach video playback handler after DOM is fully loaded
